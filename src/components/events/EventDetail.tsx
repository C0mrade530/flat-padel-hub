...
interface EventDetailProps {
  ...
  registrationStatus?: 'confirmed' | 'waiting' | null;
            ...
            <div className="glass-card rounded-t-3xl p-6 pt-4 safe-bottom">
              ...
              {registrationStatus === 'confirmed' ? (
                <div className="space-y-3">
                  <GlassCard className="p-4 text-center border-success/20" hover={false}>
                    <span className="text-success font-medium">✅ Вы записаны!</span>
                  </GlassCard>
                  <GlassButton variant="danger" size="md" fullWidth onClick={handleCancel} loading={isLoading}>
                    Отменить запись
                  </GlassButton>
                </div>
              ) : registrationStatus === 'waiting' ? (
                <div className="space-y-3">
                  <GlassCard className="p-4 text-center border-warning/20" hover={false}>
                    <span className="text-warning font-medium">⏳ Вы #{queuePosition} в очереди</span>
                  </GlassCard>
                  <GlassButton variant="ghost" size="md" fullWidth onClick={handleCancel} loading={isLoading}>
                    Покинуть очередь
                  </GlassButton>
                </div>
              ) : (
                ...
                >
                  {isFull ? "Встать в очередь" : `Записаться • ${event.price.toLocaleString("ru-RU")} ₽`}
                </GlassButton>
              )}
            </div>
          </motion.div>
